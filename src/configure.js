...
function detectForgeHints(projectDir) {
  ...
  for (const file of candidates) {
    ...
    const identity = extractLiteral(source, /identity\s*:\s*['"`]([^'"`]+)['"`]/)
    const keychainProfile = extractLiteral(
      source,
      /keychainProfile\s*:\s*['"`]([^'"`]+)['"`]/
    )
    const teamId = extractLiteral(source, /teamId\s*:\s*['"`]([^'"`]+)['"`]/)

    return { identity, keychainProfile, teamId }
  }
